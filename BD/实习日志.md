...
在vue中component不需要用响应式来包装，会影响性能

可以用markRaw来标记组件未不可响应的

```vue
markRaw(component)
```

切分支后记得重新拉一下代码，可能其他分支的还没合并过来

## 9/8

公司运行的项目用到的sass可能会比较老，可能运行npm的时候会报错
...
- **GET 请求**：参数通过 **URL 拼接** 传递（即 `?key=value&key2=value2` 的形式）。
  URL 有长度限制（不同浏览器限制不同，通常在 2KB-8KB 之间），且只能通过文本形式传递参数。
- **POST 请求**：参数通过 **请求体（Request Body）** 传递。
  请求体没有严格的长度限制，可以传递文本、二进制等多种数据格式（如 JSON、FormData、XML 等）。

**get请求**为了使得后端能够接收到正确的对象、数组参数，就需要通过序列化参数（转换为键值对key=value的形式）并拼接到URL中去。同时对特殊字符也要做处理（URL 中不允许直接出现空格、中文、`&`、`?` 等特殊字符，必须通过 `encodeURIComponent` 编码（如空格变为 `%20`，中文变为 `%E4%B8%AD%E6%96%87`）。）

而**post请求**则不需要，是因为post 可以直接传递 JSON 格式，后端可以直接解析数组，无需手动展开。且参数在请求体中独立存在，不会影响 URL 结构，因此不需要像 GET 那样拼接字符串。

**get序列化**的通用方法

```js
function serializeParamsForArray(params) {
  // 创建一个数组存储键值对部分
  const queryParts = [];
  ...
  Object.entries(params).forEach(([key, value]) => {
    ...
    if (Array.isArray(value)) {
      // 遍历数组，为每个元素创建一个 key=value 形式的键值对
      value.forEach(item => {
...
export function getBatchDownload(url, params) {
  //构建请求URL
  const queryString = serializeParamsForArray(params);
  ...
  return instance.get(fullUrl, {
    ...
    // 将二进制数据转换为 Blob 对象（浏览器可识别的二进制容器）
...
display: flex;和 display: inline-flex; 的区别在于前者是块级容器，后者是行内容器，且不会强制换行
...
handleUpload() {
      ...
      input.accept = '.pdf,.doc,.docx,.jpg,.jpeg,.png,.bmp,.mp4';//可接收的类型
      input.multiple = true;//多选
      input.onchange = async () => {
        const files = input.files;
        if (files.length === 0) return;

        const MAX_IMAGE_SIZE = 30 * 1024 * 1024;//限制上传大小最大位30MB
        const MAX_VIDEO_SIZE = 300 * 1024 * 1024;
        const MAX_OTHER_SIZE = 50 * 1024 * 1024;

        const fileModuleType = "4";
        const relatedId = this.detailData.directoryId;
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          if (mimeType.startsWith('image/')) {
            ...
            if (!fileType) {
              ...
              if (lastDotIndex > -1) {
                fileType = fileName.slice(lastDotIndex + 1).toLowerCase();
              }
            }
          }
          ...
          if (isOversize) {
            results.oversize++;
            results.oversizeFiles.push(file.name);
            results.total--;
            continue;
          }

          const formData = new FormData();
          formData.append('fileModuleType', fileModuleType);
          formData.append('relatedId', relatedId);
          formData.append('files', file);

          const promise = this.$apis.caseManager.uploadCaseFile(formData)
            .then(res => {
              if (res.code === "1") {
                results.success++;
                if (res.data.duplicateCount > 0 && res.data.duplicateFiles) {
                  results.duplicate += res.data.duplicateCount;
                  results.duplicateFiles.push(...res.data.duplicateFiles);
                }
              } else {
                results.fail++;
                results.failFiles.push(file.name);
              }
              return res;
            })
            .catch(error => {
        ...
        try {
          ...
          let message = `共上传${files.length}个文件，导入成功${results.success}个，导入失败${files.length - results.success}个`;

          if (results.duplicate > 0) {
            message += `：重名文件导入失败${results.duplicate}个：${results.duplicateFiles.join('、')}`;
          }

          if (results.oversize > 0) {
            message += `：超限文件导入失败${results.oversize}个：${results.oversizeFiles.join('、')}`;
          }

          if (results.fail > 0) {
            message += `：其他原因导入失败${results.fail}个：${results.failFiles.join('、')}`;
          }
          ...
          console.log('文件上传过程异常', error);
          // this.$message.error('文件上传过程异常');
        }
      };
      input.click();
    },
...
message 提示语的文本若是有走接口的话尽可能地用后端提供的msg字段，非接口的按情况来

## 9/28

使用window.location.href的时候，要在路由js文件中设置为平级关系才会正常跳转，且要保证js文件和vue文件的跳转路由一致

```js
handlePush (name) {
      const pathMap = {
        bigScreenDataAnalysis: '/bigScreen/dataAnalysis',
        bigScreenOnlineService: '/bigScreen/onlineService',
        bigScreenOfflineService: '/bigScreen/offlineService'
      }
      const basePath = process.env.VUE_APP_publicPath || '/'
      window.location.href = `${basePath}${pathMap[name]}`
    }
    ...
    // 大屏首页
  {
    path: '/bigScreen',
    name: 'bigScreen',
    component: () => import('@/views/bigScreen/firstScreen.vue')
  },
  // 数据分析大屏
  {
    path: '/bigScreen/dataAnalysis',
    name: 'bigScreenDataAnalysis',
    component: () => import('@/views/bigScreen/dataAnalysis/index.vue')
  },
  // 线下服务分析大屏
  {
    path: '/bigScreen/offlineService',
    name: 'bigScreenOfflineService',
    component: () => import('@/views/bigScreen/offlineServiceAnalysis/index.vue')
  },
  // 线上服务分析大屏
  {
    path: '/bigScreen/onlineService',
    name: 'bigScreenOnlineService',
    component: () => import('@/views/bigScreen/onlineServiceAnalysis/index.vue')
  },
```
